...
TRAINING_ROWS = [
    ...
    ["17:30", "訓前餐", "—", "180 g", "約 0.55 杯", "樂維根麵茶植物蛋白 1 份（40 g）＋香蕉 1 根", "—", "盡量距訓練 1.5–2.5 小時"],
    ["19:30–21:00", "重訓", "—", "—", "—", "水", "—", "依實際訓練時間前後平移"],
    ["21:00", "訓後晚餐", "150 g（半盒）", "160 g", "約 1/2 杯", "全蛋 2 顆", "200 g", "晚間吃白飯沒有問題"],
    ["睡前", "視需要補蛋白", "—", "—", "—", "樂維根麵茶植物蛋白 0.5–1 份", "—", "當天蛋白質足夠可省略"],
]
...
SHOPPING_ROWS = [
    ...
    ["香蕉", "至少 4 根"],
    ["青菜", "約 3.4 kg/週以上"],
    ["樂維根麵茶植物蛋白", "約 11–14 份/週"],
]

REPLACEMENT_ROWS = [
    ["711 鮪魚飯糰", "白飯＋部分蛋白質", "1 顆", "可放午餐或下午茶；蛋白質不足時再補樂維根麵茶植物蛋白或無糖豆漿", "依包裝", "依包裝", "依包裝", "不同口味與大小以包裝標示為準"],
...
def build_menu_sheet(workbook):
    ...
    worksheet.cell(total_row, 6, "全蛋 4 顆＋樂維根麵茶植物蛋白 1.5–2 份＋香蕉 1 根")
    ...
    worksheet.cell(rest_total, 6, "全蛋 4 顆＋樂維根麵茶植物蛋白 1.5–2 份")
